...
def proc_tceq_formatted_ethane_2025():
    ...
    with resources.path(
        "test_data", "202503_kc_autogc_formatted_tceq_ethane.txt"
    ) as test_file:
    ...
    df = df.with_columns(
        pl.when(pl.col("variable").str.contains("duplicated_0"))
        .then(pl.col("variable").str.strip_suffix(":00_duplicated_0"))
        .otherwise(pl.col("variable").str.strip_suffix(":00"))
        .alias("hour")
    )
    ...
    df = (
        df.filter(
            pl.col("Datetime").is_between(
                pl.datetime(2025, 4, 8, time_zone="Etc/GMT+6"),
                pl.datetime(2025, 4, 20, 23, time_zone="Etc/GMT+6"),
            )
        )
        .sort("Datetime")
        .upsample("Datetime", every="1h", maintain_order=True)
    )
    df2 = (
        df2.filter(
            pl.col("Datetime").is_between(
